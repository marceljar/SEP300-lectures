...
with engine.connect() as conn:
    ...
    result = conn.execute(
        text("SELECT first_name, last_name, balance "
             "FROM users WHERE balance >= :min_balance \
...
with engine.begin() as conn:
    conn.execute(
        text("UPDATE users SET balance = balance + :inc\
              WHERE user_id = :uid"),
        {"inc": 10.00, "uid": 1}
    )
    conn.execute(
        text("UPDATE users SET last_name = :new_last_name \
              WHERE user_id = :uid"),
             {"new_last_name": "Thompson", "uid": 2}
    )
...
with engine.connect() as conn:
    ...
    result = conn.execute(
        text("SELECT user_id, first_name, last_name, balance "
             "FROM users WHERE user_id IN (:u1, :u2)\
              ORDER BY user_id"),
        {"u1": 1, "u2": 2}
    )
    for row in result:
